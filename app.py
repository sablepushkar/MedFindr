...
def render_drug_result(result: dict[str, Any]) -> None:
    ...
    if status == "error":
        ...
        return
    if status == "not_found":
        st.info(result.get("message", "No matching public label information was found."))
        return
    if status != "success":
        st.info(result.get("message", "No public label information was returned."))
        return
    st.markdown("#### Retrieved public label information")
...
def render_structured_response(response: Any, view_mode: str) -> None:
    ...
    if response.drug_result is not None:
        st.markdown("### 8. Retrieved Public Label Information")
    ...
    if response.evidence:
        ...
        for item in response.evidence:
            with st.expander(item.title):
                ...
                if item.claim:
                    st.write(f"**What it supports:** {item.claim}")
                if item.provenance:
                    st.caption(item.provenance)
                if item.source_url:
                    st.caption(item.source_url)
                if item.retrieved_at_utc:
                    st.caption(f"Retrieved at (UTC): {item.retrieved_at_utc}")
    ...
    if view_mode == "technical":
        ...
        with st.expander("Development analysis trace"):
            st.json(response.trace.to_dict() if response.trace else {})
        if response.ebm is not None:
            with st.expander("Technical model detail"):
...
def main() -> None:
    ...
    view_mode = st.radio(
        ...
        ["General", "Technical"],
        horizontal=True,
        help="Technical view exposes structured data, development trace, and model details.",
    ).lower()

    samples = load_sample_concerns()
    with st.sidebar:
        st.header("Sample concerns")
        st.caption("Use a sample to inspect the SignalGraph workflow.")
    ...
    with st.form("medfindr_form", clear_on_submit=False):
        ...
        drug_name = st.text_input(
            "Optional public-label lookup term",
            max_chars=120,
            placeholder="Optional label lookup term",
    ...
    if submitted:
        ...
        with st.spinner("Running the structured information pipeline..."):
            try:
                ...
                st.error("MedFindr could not complete this request. Please try again.")
                return
        st.session_state["concern_input"] = concern
        st.subheader("SignalGraph result")
        render_structured_response(response, view_mode)
