...
def build_response(concern: str, drug_name: Optional[str] = None, include_ebm: bool = True) -> StructuredResponse:
    """Run the bounded, in-memory SignalGraph workflow."""
    concern_validation = validate_concern(concern)
    if not concern_validation.valid:
        urgency = assess_urgency("")
        trace = AnalysisTrace()
        trace.add("validation", "failed", "Concern input rejected before analysis.")
        return StructuredResponse(
            concern="", urgency=urgency,
            sections=[Section("1. Input Validation", concern_validation.error, "error")],
            notes="No analysis was performed because the concern input was invalid.",
            errors=[concern_validation.error], trace=trace,
        )
    ...
    trace = AnalysisTrace()
    trace.add("validation", "passed", "Bounded and normalized input accepted.")
    ...
    quality = assess_data_quality(context, cleaned_drug or None)
    trace.add("data_quality", "completed", f"Prototype input completeness: {quality.score}%.")
    urgency = assess_urgency(cleaned)
    trace.add("urgency", "completed", f"Prototype informational flag: {urgency.level}.")
    signal = assess_safety_signal(context)
    trace.add("safety_signal", signal.status, signal.signal_type)
    ...
    sections = [
        ...
        Section("2. Structured Findings", "MedFindr converted the supplied text into a small normalized information model.",
                items=[
                    f"Symptoms detected: {', '.join(f.concept for f in context.symptoms) or 'none'}",
                    f"Medication context: {cleaned_drug or 'not supplied'}",
                    f"Temporal medication relationship: {context.temporal_relationship}",
                ]),
        Section("3. Data Quality", quality.label,
                items=[
                    f"Completeness indicator: {quality.score}%",
                    f"Available: {', '.join(quality.present)}",
                    f"Missing/unclear: {', '.join(quality.missing) or 'none in the prototype model'}",
                ]),
    ...
    if ebm:
        ...
        factors.extend(f"{key}: {value:.2f}" for key, value in ebm.local_contribution.items())
    ...
    if drug_validation and not drug_validation.valid:
        ...
        trace.add("external_retrieval", "rejected", "Drug lookup input failed validation.")
        ...
        if drug_result.get("status") == "success":
            evidence.append(retrieved_evidence(drug_result))
    else:
        trace.add("external_retrieval", "skipped", "No optional drug lookup requested.")

    notes = (
        "MedFindr v1.2 introduces SignalGraph: a lightweight pipeline that converts supplied text into structured findings, "
        "prototype medication-related safety signals, evidence objects, data-quality indicators, and an in-memory development trace. "
        "These are software signals for information review, not diagnoses, causality assessments, treatment recommendations, or "
        "clinical validation. Retrieved label information remains separate from MedFindr-generated analysis."
    )
    return StructuredResponse(
        concern=cleaned, urgency=urgency, ebm=ebm, sections=sections, drug_result=drug_result,
        notes=notes, errors=errors, clinical_context=context, data_quality=quality,
        safety_signal=signal, evidence=evidence, trace=trace,
    )
